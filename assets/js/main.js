...
document.addEventListener("DOMContentLoaded", function () {
  ...
  menuItems.forEach((item) => {
    item.addEventListener("click", function () {
      checkbox.checked = false; // Uncheck the checkbox when a menu item is clicked
    });
  });
});

//DONATION PAGE
// Helper: set amount via button
function setAmount(value) {
  document.getElementById('amount').value = value;
}

// Prefill donation purpose from URL param (e.g., donate.html?cause=medical)
function setDonationPurposeFromURL() {
  ...
  if (cause) {
    ...
    const matchingOption = options.find(opt => opt.value.toLowerCase().includes(cause.toLowerCase()));
    if (matchingOption) {
